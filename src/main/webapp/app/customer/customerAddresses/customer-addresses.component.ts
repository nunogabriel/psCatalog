...
@Component({
    selector: 'jhi-customer-addresses',
...
export class CustomerAddressesComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.customerAddressesService
                ...
                .subscribe(
                    (res: HttpResponse<ICustomerAddresses[]>) => this.paginateCustomerAddresses(res.body, res.headers),
    ...
    trackId(index: number, item: ICustomerAddresses) {
        return item.id;
    }

    registerChangeInCustomerAddresses() {
        this.eventSubscriber = this.eventManager.subscribe('customerAddressesListModification', response => this.loadAll());
    }
    ...
    private paginateCustomerAddresses(data: ICustomerAddresses[], headers: HttpHeaders) {
